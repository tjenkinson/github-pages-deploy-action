    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (util_1.isNullOrUndefined(constants_1.action.accessToken) &&
                ...
                util_1.isNullOrUndefined(constants_1.action.deployKey)) {
            ...
            if (!util_1.isNullOrUndefined(constants_1.action.deployKey)) {
                const sshDirectory = `${constants_1.workspace}/.ssh`;
                yield execute_1.execute(`mkdir ${sshDirectory}`, constants_1.workspace);
                yield execute_1.execute(`ssh-keyscan github.com >> ${sshDirectory}/known_hosts`, constants_1.workspace);
                yield execute_1.execute(`echo "${constants_1.action.deployKey}" > "${sshDirectory}/id_rsa"`, constants_1.workspace);
                yield execute_1.execute(`chmod 400 ${sshDirectory}/id_rsa`, constants_1.workspace);
            }
