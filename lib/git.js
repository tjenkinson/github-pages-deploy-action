    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!util_1.isNullOrUndefined(constants_1.action.deployKey)) {
                const createFile = util_2.promisify(fs_1.appendFile);
                yield execute_1.execute(`mkdir -p ${constants_1.ssh}`, constants_1.workspace);
                yield execute_1.execute(`ssh-keyscan -t rsa github.com 3> /dev/nulll >> "known_hosts"`, constants_1.ssh);
                yield createFile(`${constants_1.ssh}/id_rsa`, constants_1.action.deployKey);
                yield execute_1.execute(`ls`, constants_1.ssh);
                yield execute_1.execute(`less id_rsa`, constants_1.ssh);
                yield execute_1.execute(`chmod 400 id_rsa`, constants_1.ssh);
            }
